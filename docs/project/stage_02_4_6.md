...
# CORE FRONTIER
# Stage 02.4.6
# Developer Recovery Report
# HUD Refactor Failure + Runtime Recovery

## Событие

Во время исполнения Stage 02.4.6 был предпринят крупный UI/HUD refactor с одновременным изменением нескольких взаимосвязанных файлов интерфейса.
...
Также предпринимались промежуточные попытки:
- перестройки HUD,
- смены расположения панелей,
- вертикального/горизонтального adaptive layout,
- переноса speed controls,
- изменения логики меню и overlay.

---

## Причина сбоя

Главная ошибка заключалась в том, что изменения производились сразу в нескольких связанных runtime-файлах без строгой dependency map.

В результате:
- UI-система потеряла совместимость,
- панели начали конфликтовать,
- overlay перекрывал runtime,
- controls перестали корректно обрабатываться,
- часть интерфейса уходила за экран,
- происходил partial runtime collapse.

На мобильных устройствах наблюдалось:
- зелёный экран,
- отсутствие карты,
- неработающие кнопки,
- перестройка HUD при нажатии speed controls,
- потеря управления.
...
Во время восстановления возникла путаница между:
- baseline-файлами,
- recovery-файлами,
- patched-файлами,
- partially modified runtime.

Это произошло из-за отсутствия:
- жёсткой идентификации версий внутри файлов,
- строгой фиксации active baseline,
- dependency tracking между UI subsystem файлами.

---

## Recovery Process

Для восстановления был использован:
- последний стабильный ZIP baseline,
- ручной rollback runtime-файлов,
- полная замена поврежденных subsystem файлов.
...
- canvas снова отображается,
- build mode работает,
- placement работает,
- HUD снова функционирует,
- speed controls работают стабильно,
- игра снова запускается.

---

## Главный вывод Stage 02.4.6

Массовый UI refactor без:
- isolated subsystem,
- dependency map,
- rollback discipline,
- compatibility testing

привёл к потере стабильности runtime.

---

## Новые правила проекта

Теперь вводятся ограничения:

1. Только один стабильный baseline.
2. Перед любым изменением создаётся ZIP snapshot.
3. Изменения производятся малыми patch-pass.
4. Не допускается одновременное изменение:
   - state.js
   - layout.js
   - controls.js
   - panels.js
5. Каждый pass должен быть:
   - локальным,
   - обратимым,
   - быстро тестируемым.
6. Каждый файл обязан содержать:
   - PROJECT
   - STAGE
   - FILE ROLE
   - PASS TYPE
...
- runtime восстановлен,
- baseline сохранён,
- проект снова работоспособен,
- дальнейшая разработка продолжается только через small compatibility-safe patches.